...
- (TRSBaseModel *)configPageModelWith:(NSDictionary *)properties{
    ...
    if (properties) {
        ...
        [dataDic setObject:@"A0010" forKey:@"se_code"];
    }
    
    if (!TRSBlankStr(self.refer)) [dataDic setObject:self.refer forKey:@"refer"];
    
    if (self.pv) [dataDic setObject:self.pv forKey:@"pv"];
    if (self.vt) [dataDic setObject:self.vt forKey:@"vt"];
    [dataDic setObject:[NSString stringWithFormat:@"%lld", [self.pageEndTime longLongValue]-[self.pageBeginTime longLongValue]>0?[self.pageEndTime longLongValue]-[self.pageBeginTime longLongValue]:0] forKey:@"dur"];
//    if (self.vt) [dataDic setObject:self.vt forKey:@"se_vt"];
    if (self.pageName) [dataDic setObject:self.pageName forKey:@"se_pt"];
...
- (NSDictionary *)dicWithOriginalDic:(NSDictionary *)dic{
    ...
    if (TRSBlankStr(dic[@"se_code"])) { // 为空页面添加A0010标识
        [resultDic setObject:@"A0010" forKey:@"se_code"];
    }
